...
const PurchaseRequisitions = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        console.log('🔄 Fetching inventory items and requisitions...');
        ...
        console.log('📦 Inventory items fetched:', inventoryRes.data);
        console.log('📊 Inventory items type:', typeof inventoryRes.data);
        console.log('📏 Inventory items length:', inventoryRes.data?.length);
  ...
  const deleteRequisition = async (reqId) => {
    ...
    try {
      console.log('📤 Sending DELETE request to:', `/purchase-requisitions/${reqId}`);
      ...
      console.log('🔄 Refreshing inventory after delete...');
      const inventoryRes = await api.get('/inventory/');
      console.log('📦 Inventory after delete:', inventoryRes.data);
      const activeItems = inventoryRes.data?.filter(item => item.is_active !== false) || [];
      setInventoryItems(activeItems);
    } catch (error) {
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">
                  {requisitions.filter(r => r.status === 'PENDING' || r.status === 'APPROVED').length}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="p-4 border-b flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {requisitions.filter(r => r.status === 'PENDING' || r.status === 'APPROVED').length} pending, {requisitions.length} total
            </p>
          </div>
